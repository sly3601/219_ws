...
StateTrotting::StateTrotting(CtrlInterfaces &ctrl_interfaces,
    ...
    Kpp = Vec3(22, 22, 300.1).asDiagonal();
...
void StateTrotting::calcTau() {
    ...
    */
    else if(troting_kalman == 2)
    {
        ...
        // ========== 限制 roll， pitch，yaw ==========
        d_wbd(0) = saturation(d_wbd(0), Vec2(-60, 60));
        d_wbd(1) = saturation(d_wbd(1), Vec2(-70, 70));
        d_wbd(2) = saturation(d_wbd(2), Vec2(-36, 36));
        ...
        const double tau_ff_limit_hip = 34.0;
...
void StateTrotting::calcQQd() {
    ...
    const double hip_range  = 0.16;   // 先用 ±0.08 rad（约 ±4.6°）
