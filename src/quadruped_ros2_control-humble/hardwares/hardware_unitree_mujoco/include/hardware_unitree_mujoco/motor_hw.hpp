...
#define Motor_id uint32_t
    ...
    typedef struct can_send_frame  // 该结构体的顺序是严格不能变的
    {
        ...
        void modify(const Motor_id id, const uint8_t* send_data) // C++ 中，结构体 和 类 的非静态成员函数（比如这里的 modify） 不会占用任何对象的内存
        {
            canId = id;
            std::copy(send_data, send_data+8, data);  // std::copy(源起始位置, 源结束位置, 目标起始位置);
        }
...
#pragma pack()
    ...
    private:
        ...
        float state_dq=0;
        float state_tau=0;
        Limit_param limit_param{}; // min max等限制参数
